...
(define (generate-string-from-reg-exp exp)
  (cond
    ...
    [(either? exp) (if (equal? 1 (pick '(1 2))) (generate-string-from-reg-exp (either-arg1 exp)) (generate-string-from-reg-exp (either-arg2 exp)))]
    [(repeat? exp) (repeater (repeat-arg1 exp))]
    [(list? exp) exp]))

(define (repeater exp)
  (if (flip .5)
      (append (generate-string-from-reg-exp exp) (repeater exp))
      '()))
...
(define (dfa-accepts? mach str)
  (if (member (dfa-helper mach str (dfa-start-state mach)) (dfa-accepting-states mach)) #t #f))

; Helper that changes the state after iterating each symbol in the string
(define (dfa-helper mach str state)
  (cond
    [(empty? str) state]
    [else (dfa-helper mach (cdr str) (lookup (append (list state) (list (car str))) (dfa-transitions mach)))]))


; Helper from hw6.rkt that just retrieves value of an entry with matching key
...
(define (list-leaf-labels-helper tree)
  (cond
    ...
    [(node? tree) (map (lambda (x) (list-leaf-labels-helper x)) (node-children tree))]))
...
; Helper that creates the tree by creating leaves for terminals or recursively calling function on all arguments in randomly picked rule-rhs of a state
(define (generate-helper grammar state)
  (cond
    [(member state (cfg-terminals grammar)) (leaf state)]
    [else (node state (map (lambda (x) (generate-helper grammar x)) (pick (rule-helper state (cfg-rules grammar)))))]))


; Helper that returns a list of all the rhs's associated with a lhs; similar to lookup helper
(define (rule-helper lhs rules [result '()])
  (cond
    [(empty? rules) result]
    [(equal? lhs (rule-lhs (car rules))) (append (list (rule-rhs (car rules))) (rule-helper lhs (cdr rules)))]
    [else (rule-helper lhs (cdr rules))]))
...
(define exp-mcd
  (concat (either '(it)
          ...
          (concat (repeat (concat (concat (either '(dreamed) '(believed)) '(that))
                                  (either '(it)
                                          (concat (either '(a) '(the))
                                                  (either '(mouse) (either '(cat) '(dog)))))))
                  (either (either '(swam) '(slept))
                          (concat (either '(evaded) '(chased))
                                  (either '(it)
                                          (concat (either '(a) '(the))
                                                  (either '(mouse) (either '(cat) '(dog))))))))))
...
(define my-cfg
  (cfg
   '(the a strong fast talented quarterback runningback receiver ball touchdown that scored ran threw thought knew)
   '(start det n n2 n3 adj v v2 v3)
   'start
   (list
    (rule 'start '(det n v))
    (rule 'det '(the))
...
; Example sentences:
;   the runningback scored
;   a fast receiver ran
;   the talented quarterback threw the touchdown
;   a receiver thought that the fast runningback ran the ball
;   the fast runningback thought that the talented quarterback knew that a receiver scored

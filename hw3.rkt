...
; My Turing machine works by first getting to the right end of the tape.
; It replaces the symbol at the end of the tape with an 'x, then goes to the right until it reaches a blank.
; The blank on the right end is replaced by whatever symbol was changed into the 'x.
; The head then travels left until it reaches the next 0 or 1 and repeats the process.
; After replacing all the 'x with blanks, the output is the reversed input.

(define tm-reverse
  (list
   ; Moves to right end of the tape
   (ins 'q1 0 'q1 0 'R)
   (ins 'q1 1 'q1 1 'R)
   (ins 'q1 'b 'q2 'b 'L)
   ; Skips over all the other 'x and turns the next 0 or 1 into 'x
   (ins 'q2 0 'q3 'x 'R)
   (ins 'q2 1 'q4 'x 'R)
   (ins 'q2 'x 'q2 'x 'L)
   (ins 'q2 'b 'q6 'b 'R)
   ; Goes to the right end and replaces blank with 0 (replaced symbol from 'q2)
   (ins 'q3 'x 'q3 'x 'R)
   (ins 'q3 1 'q3 1 'R)
   (ins 'q3 0 'q3 0 'R)
   (ins 'q3 'b 'q5 0 'L)
   ; Goes to the right end and replaces blank with 1 (replaced symbol from 'q2)
   (ins 'q4 'b 'q5 1 'L)
   (ins 'q4 1 'q4 1 'R)
   (ins 'q4 0 'q4 0 'R)
   (ins 'q4 'x 'q4 'x 'R)
   ; Goes left until it finds first 'x then goes to 'q2
   (ins 'q5 'x 'q2 'x 'L)
   (ins 'q5 1 'q5 1 'L)
   (ins 'q5 0 'q5 0 'L)
   ; Replaces all the 'x with 'b
   (ins 'q6 'x 'q6 'b 'R)))
; ****************************************************************
; ** problem 2 (10 points)
...
(define (i-match? state symbol inst)
  (if (and (equal? state (ins-c-state inst)) (equal? symbol (ins-c-symbol inst))) #t #f))

(define (i-lookup state symbol mach)
  (cond
    [(empty? mach) #f]
    [(i-match? state symbol (list-ref mach 0)) (list-ref mach 0)]
...
(define (halted? mach config)
  (if (equal? #f (i-lookup (conf-state config) (conf-symbol config) mach)) #t #f))
...
; My helper function removes blanks from beginning of a list
; Examples:
;   (blanker '(b b 1)) => '(1)
;   (blanker '(1 0 b)) => '(1 0 b)
; Use list-ref for conf-ltape and conf-rtape

(define (blanker lst)
  (cond
    [(empty? lst) '()]
    [(not (equal? 'b (first lst))) lst]
    [else (blanker (rest lst))]))

(define (normalize config)
  (conf (conf-state config)
        (blanker (conf-ltape config))
        (conf-symbol config)
        (reverse (blanker (reverse (conf-rtape config))))))
...
(define (shift-head-left config)
  (conf (conf-state config)
        ...
        (if (and (empty? (conf-rtape config)) (equal? 'b (conf-symbol config))) '() (cons (conf-symbol config) (conf-rtape config)))))

(define (shift-head-right config)
  (conf (conf-state config)
        (if (and (empty? (conf-ltape config)) (equal? 'b (conf-symbol config))) '() (append (conf-ltape config) (list (conf-symbol config))))
        (if (empty? (conf-rtape config)) 'b (first (conf-rtape config)))
        (if (empty? (conf-rtape config)) '() (rest (conf-rtape config)))))
...
(define (next-config mach config)
  (cond
    ...
    [else (if (equal? 'R (ins-dir (i-lookup (conf-state config) (conf-symbol config) mach)))
              (shift-head-right (write-symbol (ins-n-symbol (i-lookup (conf-state config) (conf-symbol config) mach)) (change-state (ins-n-state (i-lookup (conf-state config) (conf-symbol config) mach)) config)))
              (shift-head-left (write-symbol (ins-n-symbol (i-lookup (conf-state config) (conf-symbol config) mach)) (change-state (ins-n-state (i-lookup (conf-state config) (conf-symbol config) mach)) config))))]))
...
; My Turing machine works by first moving to the right end of the tape, placing 'p for a placeholder at the first blank.
; Then, it subtracts one from the binary number and adds an 'x on the right side of the 'p.
; This repeats until the binary number is zero.
; Finally, replaces everything besides 'x with blanks

(define tm-convert
  (list
   ; Moves to right end of tape
   (ins 'q1 1 'q1 1 'R)
   (ins 'q1 0 'q1 0 'R)
   (ins 'q1 'b 'q2 'p 'L)
   ; Subtract 1 from the binary number (turn first 1 from the right into a 0)
   (ins 'q2 0 'q2 0 'L)
   (ins 'q2 'x 'q2 'x 'L)
   (ins 'q2 'p 'q2 'p 'L)
   (ins 'q2 1 'q3 0 'R)
   (ins 'q2 'b 'q5 'b 'R)
   ; Change trailing 0's to 1's (for the subtraction of binary number)
   (ins 'q3 0 'q3 1 'R)
   (ins 'q3 'p 'q4 'p 'R)
   ; Replaces 'b with 'x at right end
   (ins 'q4 'b 'q2 'x 'L)
   (ins 'q4 'x 'q4 'x 'R)
   ; Replaces the now zero binary number and 'p with blanks, leaving just the 'x symbols
   (ins 'q5 0 'q5 'b 'R)
   (ins 'q5 'p 'q6 'b 'R)))
...
; My Turing machine works by first moving to the right end of the tape, placing 'p for a placeholder at the first blank.
; Then, it finds the first 0 from the right and replaces it with an 'x.
; Next, it places that 0 at the right end of the tape.
; Next, it takes you back to the placeholder so no copied 0's accidentally get turned to 'x.
; This repeats until it hits a blank at the left end of the tape (no 0's left on the left of 'p).
; Then, it goes back to the placeholder 'p and repeats the whole process for the 1's.
; In the end, it replaces all the 'x and 'p with 'b.
...
(define tm-sort
  (list
   ...
   (ins 'q1 0 'q1 0 'R)
   (ins 'q1 'b 'q2 'p 'L)
   ; Finds a 0 and replaces it with 'x
   (ins 'q2 1 'q2 1 'L)
   (ins 'q2 'x 'q2 'x 'L)
   (ins 'q2 0 'q3 'x 'R)
   (ins 'q2 'b 'q5 'b 'R)
   ; Places a 0 at the right end of the tape
   (ins 'q3 0 'q3 0 'R)
   (ins 'q3 1 'q3 1 'R)
   (ins 'q3 'p 'q3 'p 'R)
   (ins 'q3 'x 'q3 'x 'R)
   (ins 'q3 'b 'q4 0 'L)
   ; Takes you back to the placeholder so no copied 0's get turned to 'x from 'q2
   (ins 'q4 'p 'q2 'p 'L)
   (ins 'q4 0 'q4 0 'L)
   ; Goes to placeholder
   (ins 'q5 1 'q5 1 'R)
   (ins 'q5 'x 'q5 'x 'R)
   (ins 'q5 'p 'q6 'p 'L)
   ; Finds a 1 and replaces it with 'x
   (ins 'q6 'x 'q6 'x 'L)
   (ins 'q6 1 'q7 'x 'R)
   (ins 'q6 'b 'q9 'b 'R)
   ; Places a 1 at the right end of the tape
   (ins 'q7 0 'q7 0 'R)
   (ins 'q7 1 'q7 1 'R)
   (ins 'q7 'p 'q7 'p 'R)
   (ins 'q7 'x 'q7 'x 'R)
   (ins 'q7 'b 'q8 1 'L)
   ; Takes you back to placeholder so no copied 1's get turned to 'x from 'q6
   (ins 'q8 'p 'q6 'p 'L)
   (ins 'q8 0 'q8 0 'L)
   (ins 'q8 1 'q8 1 'L)
   ; Replaces all 'x and 'p with 'b
   (ins 'q9 'x 'q9 'b 'R)
   (ins 'q9 'p 'q10 'b 'R)))
